    ...
    (https, record, runtime, dialog, message, serverWidget, url) => {
        ...
        const beforeLoad = (scriptContext) => {
            try
            {
                ...
                log.debug("My car is VIA CLASS CONSTRUCTOR" + myCar.age() + " years old.");

                var myCar = getCar("Ford", 2014);
                log.debug("My car is VIA FUNCTIONS" + myCar.age() + " years old.");

                //classes are good way to group methods
                //but you are also somehow being limited from reusing these methods(private methods), not an issue if everything is well planned
                //when you break a snippet on the class, you may break the whole class
                //when you break a standalone function you only break that function
                ...
                //Most don’t know this, but under the hood, all classes are functions, and all functions are objects. (Arrays are objects, too, but that’s besides the point.) As a result, you are better off in most cases just writing objects and functions directly as it is more behaviorally consistent, more pleasant to maintain, less cumbersome to write, and easier to read.
            }
            catch(e)
            {
